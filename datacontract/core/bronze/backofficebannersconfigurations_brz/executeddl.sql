...
CREATE OR REPLACE TABLE ${env:ENV_CATALOG_NAME_CORE}.bronze.backofficebannersconfigurations_brz (
  end_date STRING COMMENT "Fecha de finalización de la vigencia del banner, configurada en el backoffice. El valor se encuentra en formato UTC-0. Formato: dd/MM/yyyy HH:mm:ssZ.",
  owner STRING COMMENT "Owner provider configurado en el backoffice, utilizado para identificar a los usuarios a quienes se mostrará el banner según el owner provider. Este valor solo está presente cuando el banner es de tipo MASIVO. Valores posibles: BCP, NIUBIZ, YAPEDIGITAL, YAPEEMPRESA.",
  product STRING COMMENT "Producto asociado al home banner, empleado para los clusters de NBP (Next Best Product) en el futuro.",
  __deleted STRING COMMENT "Campo utilizado como convención de integración con Debezium. False indica que el registro no ha sido eliminado (registro activo), mientras que True indica que el registro ha sido eliminado (tombstone).",
  deeplink STRING COMMENT "Deeplink utilizado en el banner para redireccionar a los usuarios cuando interactúan con él.",
  blacklist STRING COMMENT "Corresponde a un arreglo de valores que indica la lista de segmentos que actuarán como blacklist dentro de la lógica del API Connect Hub. Si un segmento está incluido en esta lista, los usuarios pertenecientes a ese segmento no visualizarán el banner configurado.",
  ...
  created_at STRING COMMENT "Fecha de creación del home banner. El valor se encuentra en formato UTC-0. Formato: dd/MM/yyyy HH:mm:ssZ.",
  ...
  banner_type_position STRING COMMENT "Indica el tipo de configuración de segmentación del banner. Los valores determinan si es de tipo MASIVO (envíos según owner provider del usuario), SEGMENTADO (segmentación personalizada por bases), NBP (personalización a nivel de usuario) o DEFAULT (banners por defecto si un usuario no tiene banners pendientes).",
  ...
  updated_at STRING COMMENT "Fecha de modificación de un registro del banner. El valor se encuentra en formato UTC-0. Formato: dd/MM/yyyy HH:mm:ssZ.",
  segment STRING COMMENT "Corresponde a un arreglo de valores que indica el listado de segmentos a los cuales se disponibilizará el banner. Este valor solo está presente cuando el banner es de tipo SEGMENTADO.",
  id STRING COMMENT "Identificador único del banner creado desde el backoffice, utilizado como clave primaria (PK).",
  phone_donation STRING COMMENT "Número de celular de la ONG a la cual se realizarán los yapeos donativos. Este campo solo tiene valor si el banner es de tipo donación.",
  position STRING COMMENT "Posición relativa que tendrá el banner dentro del carrusel o listado de banners en los flujos que lo soporten.",
  view_scope STRING COMMENT "Indica la pantalla en la que debe aparecer el banner configurado en el backoffice.",
  start_date STRING COMMENT "Fecha de inicio de la publicación del banner, configurada en el backoffice. El valor se encuentra en formato UTC-0. Formato: dd/MM/yyyy HH:mm:ssZ.",
  status STRING COMMENT "Indica el estado del registro del banner. Valores posibles: BORRADOR (el registro aún se encuentra en borrador y puede ser modificado), VALIDACION (el registro se encuentra en validación por un owner), APROBADO (el registro está activado), ELIMINADO (el registro ha sido eliminado desde el backoffice).",
  metadata STRUCT<
...
COMMENT "Descripción de la tabla: Esta tabla almacena la configuración y atributos de los banners gestionados desde el backoffice, permitiendo su segmentación, personalización y control de visibilidad según tipo, producto, segmento de usuario y estado. Incluye información relevante para la gestión de banners funcionales y de donación, así como detalles de accesibilidad, fechas de vigencia, segmentación, y control de eliminación lógica."
 TBLPROPERTIES (
