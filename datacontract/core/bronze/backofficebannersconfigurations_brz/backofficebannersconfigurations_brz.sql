...
CREATE OR REPLACE TABLE core.bronze.backofficebannersconfigurations_brz (
  ...
  blacklist STRING COMMENT "Corresponde a un arreglo de valores. Este campo indica la lista de segmentos que funcionarán como blacklist dentro de la lógica del API Connect Hub; si un segmento está incluido, los usuarios pertenecientes a ese segmento no visualizarán el banner configurado.",
  ...
  organization_name STRING COMMENT "Nombre de la ONG u organización utilizado para crear el banner de tipo donación. Este campo solo tendrá valor si el banner es de tipo donación.",
  ...
  banner_type_position STRING COMMENT "Tipo de configuración de segmentación del banner creado. Valores posibles: MASIVO (envíos según owner provider del usuario), SEGMENTADO (segmentación personalizada por bases), NBP (personalización a nivel usuario), DEFAULT (banners por defecto si el usuario no tiene banners pendientes).",
  ...
  updated_at STRING COMMENT "Fecha de modificación del registro del banner. Formato UTC-0. Formato: dd/MM/yyyy HH:mm:ssZ.",
  segment STRING COMMENT "Corresponde a un arreglo de valores. Indica el listado de segmentos a los que se disponibilizará el banner. Este valor solo se presenta cuando el banner es de tipo SEGMENTADO.",
  id STRING COMMENT "Identificador único del banner creado desde el backoffice, utilizado como clave primaria (PK).",
  phone_donation STRING COMMENT "Número de celular de la ONG a la que se realizarán los yapeos donativos. Este campo solo tendrá valor si el banner es de tipo donación.",
  ...
  view_scope STRING COMMENT "Pantalla en la que debe aparecer el banner configurado en el backoffice.",
  ...
  status STRING COMMENT "Estado del registro del banner. Valores posibles: BORRADOR (el registro está en borrador y puede ser modificado), VALIDACION (el registro está en validación por un owner), APROBADO (el registro está activado), ELIMINADO (el registro ha sido eliminado desde el backoffice).",
  metadata STRUCT<
...
COMMENT "Descripción de la tabla: Esta tabla almacena la configuración y atributos de los banners mostrados en la aplicación, permitiendo su segmentación, personalización y control de vigencia. Incluye información relevante como fechas de publicación, tipo de banner, segmentación, estado, detalles de accesibilidad, y datos específicos para banners de donación o funcionales. La gestión y visualización de estos banners se realiza a través del backoffice, permitiendo una administración flexible y dinámica de la experiencia de usuario."
 TBLPROPERTIES (
