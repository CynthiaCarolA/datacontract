-- Data Contract: urn:develop.bronze.backofficebannersconfigurations_brz-latest
-- SQL Dialect: databricks
CREATE OR REPLACE TABLE develop.bronze.backofficebannersconfigurations_brz (
...
True: El registro ha sido eliminado (es un \"tombstone\").",
  ...
  metadata STRUCT<
      ...
      offset BIGINT COMMENT "Número entero que se asigna a cada evento enviado a un EventHub.",
      sequenceNumber BIGINT COMMENT "Indica la posición secuencial de un mensaje dentro de una partición",
      enqueuedTime TIMESTAMP COMMENT "Fecha en que se encola el registros para eventHub."
    > COMMENT "Metadata from eventhub" not null,
...
ALTER TABLE develop.bronze.backofficebannersconfigurations_brz SET TAGS ('Domain'='develop', 'Layer'='bronze', 'TypeTable'='raw');
ALTER TABLE develop.bronze.backofficebannersconfigurations_brz ALTER COLUMN phone_donation SET TAGS ('dac');
